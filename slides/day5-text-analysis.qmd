...
Platform URLs often follow **predictable patterns**:

`https://www.tiktok.com/@{author_name}/video/{video_id}`

. . .

We can construct them with `mutate()` + `paste0()`:

```{r}
corona <- corona |>
  mutate(video_url = paste0(
    "https://www.tiktok.com/@", author_name,
    "/video/", video_id
  ))
```

Now every row has a clickable link to the actual video!

## From Patterns to Posts

```{r}
# Quantitative filter → URL → qualitative analysis
corona |>
  arrange(desc(view_count)) |>
  head(5) |>
...
**The workflow:** filter interesting videos with R → visit them for close reading

. . .

What can you learn from the **full multimodal post** (video, audio, comments) that you can't from metadata alone?
